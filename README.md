...
The expandable **Download creator history** area can fetch every available
video from one saved creator over the past 1 to 10 years. Historical
...
The expandable **Download a video** area accepts an individual YouTube watch,
Shorts, live, or `youtu.be` link. Playlist parameters are removed so only the
requested video is considered. The normal date and per-creator limits do not
apply, while the download archive, Plex naming and metadata, progress display,
and configured network transfer continue to apply.

The expandable **Video download log** keeps the latest 500 video records and
shows separate Download and Transfer indicators. It records new activity from
version 0.7.0 onward; older downloads are not reconstructed, although an older
pending file transferred by this version is added to the log. Hover a failed
indicator to see its error. The log is stored with the other persistent state.
...
Install on Raspberry Pi OS:
    ...
    sudo apt install ./creator-catcher_0.7.0_all.deb

Verify a downloaded release before installing:

    sha256sum -c creator-catcher_0.7.0_all.deb.sha256
...
The web interface has no application-level login in version 0.7.0. Keep it on
