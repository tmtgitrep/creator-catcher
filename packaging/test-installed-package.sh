...
if ! apt-get -qq -o Dpkg::Use-Pty=0 install -y /project/dist/creator-catcher_0.4.0_all.deb >>/tmp/creator-catcher-apt.log 2>&1; then
    cat /tmp/creator-catcher-apt.log
    exit 1
fi
test "$(dpkg-query -W -f='${Status}' creator-catcher)" = "install ok installed"
test "$(creator-catcher --version)" = "0.4.0"
